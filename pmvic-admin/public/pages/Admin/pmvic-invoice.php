...
<body>
    ...
    <div class="main-container">
                        ...
                        <div class="row">
                                ...
                                <div class="input-group mb-3">
                                    <div class="input-group-prepend">
                                        <span class="input-group-text bg-info text-white" id="basic-addon1"><i
                                                class="icon-copy bi bi-calendar-plus-fill"></i></span>
                                    </div>
                                    <input id="start_date" class="form-control date-picker"
                            ...
                            <div class="col-md-2">
                                ...
                                <div class="input-group mb-3">
                                    ...
                                    <input id="end_date" class="form-control date-picker" placeholder="Select End Date"
                                        type="text">
                                </div>
                            </div>
                            <div class="col-md-2">
                                ...
                                <div class="form-group">
                                    <div class="dropdown bootstrap-select form-control">
                                        <select class="selectpicker form-control" data-style="btn-outline-primary"
                                            data-size="5" tabindex="-98" id="select_pmvic">
                                            <option>--SELECT PMVIC CENTER--</option>
                                            <?php foreach ($pmvic_name as $row) :?>
                                            <option value="<?= $row['pmvic_name'] . ' - ' . $row['pmvic_address'] ?>"
                                                class="form-control">
                                                <?= $row['pmvic_name'] . ' - ' . $row['pmvic_address'] ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-5 mt-4">
                                <div>
                                    <button id="searchLV" class="btn btn-primary"><i class="icon-copy fa fa-filter"
                                            aria-hidden="true"></i> LV SEARCH</button>
                                    <button id="searchMC" class="btn btn-info"><i class="icon-copy fa fa-filter"
                                            aria-hidden="true"></i> MC SEARCH</button>
                                    <button id="printButton" class="btn btn-success"><i class="icon-copy fa fa-print"
                        ...
                        <div class="invoice-wrap" id="content">
                            <div class="invoice-box">
                                        ...
                                        <ul id="itemList">
                                            ...
                                            <li class="clearfix">
                                                <div class="invoice-sub">
                                                    <?= date('Y-m-d', strtotime('+' . ($i - 5) . ' day', strtotime(date('Y-m-d')))) ?>
                                                </div>
                                            ...
                                            <ul>
                                                <li class="clearfix">
                                                    ...
                                                    <div class="invoice-rate font-20 weight-600">
                                                        <p class="font-14 mb-2">
                                                            <span class="weight-500 font-24 text-black"><span
                                                                    id="totalUp">0</span></span>
                                                        </p>
                                                    </div>
                                                    <div class="invoice-subtotal">
                                                        <span class="weight-600 font-24 text-black">Php <span
                                                                id="totalDue">0.00</span></span>
                                                    </div>
                                                </li>
                                            </ul>
                                        </div>
                                ...
                                <div class="footer text-center">
                                    <h6 class="text-center pb-6">Please remit check payment payable to: <strong>SYSTECH
                                            IT SOLUTION AND SERVICES CORP.</strong></h6>
                                    <p class="pt-10"><strong>PNB ( Account #:120670003920 )</strong></p>
                                    <p> Please viber/email us the validated deposit slip within two days after the date
                                        deposited to: </p>
        ...
        <script>
        $(document).on('click', '#searchMC', function() {
            ...
            if (startdate != '' && enddate != '' && selectPmvic != '') {
                $.ajax({
                    method: "POST",
                    url: "../../../private/controller/invoice/fetch-pmvic-invoice.php",
                    data: {
                        'startdate': startdate,
                        'enddate': enddate,
                    ...
                    success: function(response) {
                        ...
                        function dateToYMD(date) {
                            var strArray = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug',
                                'Sep', 'Oct', 'Nov', 'Dec'
                            ];
                            var d = date.getDate();
                        ...
                        $('#dates').html(
                            `${dateToYMD(new Date(startdate))} - ${dateToYMD(new Date(enddate))}`
                            )
                    },
            ...
            const showListItem = (data) => {
                ...
                data.map(el => {
                    result += `
						<li class="clearfix">
							...
							<div class="invoice-hours">${el.MC_PRICE}</div>
							<div class="invoice-subtotal">
				...
				var element = document.getElementById('newtitle');
					element.value = 'random_value';
            }
        });

        $(document).on('click', '#searchLV', function() {
            ...
            if (startdate != '' && enddate != '' && selectPmvic != '') {
                $.ajax({
                    method: "POST",
                    url: "../../../private/controller/invoice/fetch-pmvic-invoice-lv.php",
                    data: {
                        'startdate': startdate,
                        'enddate': enddate,
                        'selectPmvic': arrPmvic[0].trim()
		...
		<li class="clearfix">
			<div class="invoice-sub">${el.date}</div>
			<div class="invoice-rate">${el.count}</div>
			<div class="invoice-hours">${el.LV_PRICE}</div>
			<div class="invoice-subtotal">
				<span class="weight-600">${numberWithCommas(el.total_price)}</span>
			</div>
		</li>
